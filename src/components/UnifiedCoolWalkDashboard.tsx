...
export const UnifiedCoolWalkDashboard: React.FC<UnifiedDashboardProps> = ({
  ...
  const formattedSunTime = `${String(Math.floor(timeSliderHours)).padStart(2, '0')}:${String(Math.floor((timeSliderHours % 1) * 60)).padStart(2, '0')}`;
    ...
    <div className="h-full w-full bg-[#070709] text-zinc-100 overflow-y-auto select-none">
      <div className="max-w-7xl mx-auto p-3 sm:p-5 lg:p-6 space-y-6">
        ...
        {/* =========================================================================
           ...
           ========================================================================= */}
        <section aria-labelledby="observatory-heading" className="bg-[#101014] border border-zinc-800/80 rounded-2xl p-4 sm:p-5 shadow-2xl backdrop-blur-md">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <h1 id="observatory-heading" className="text-xl sm:text-2xl font-black text-white tracking-tight flex items-center gap-2">
                  <Navigation className="w-5 h-5 text-emerald-400" aria-hidden="true" />
                  CoolWalk — {originPoint.name}
                </h1>
                
                {weather && (
                  <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 px-3 py-1 rounded-xl" aria-label={`Current weather: ${weather.temperatureC} degrees Celsius, ${weather.weatherDescription}`}>
                    <Thermometer className="w-4 h-4 text-rose-400" aria-hidden="true" />
                    <span className="text-base font-bold text-white font-mono">{weather.temperatureC}°C</span>
                    <span className="text-xs text-zinc-300 font-mono">({weather.weatherDescription})</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap" role="toolbar" aria-label="Location controls">
              <button
              ...
              >
                <Crosshair className={`w-3.5 h-3.5 ${isLocating ? 'animate-spin' : ''}`} aria-hidden="true" />
                <span>{isLocating ? 'Acquiring GPS...' : 'Use My GPS'}</span>
              </button>

              <button
                onClick={onOpenLocationModal}
                aria-label="Search global cities and coordinates"
              ...
              {/* Preset City Chips */}
              <div className="hidden xl:flex items-center gap-1.5 pl-2 border-l border-zinc-800" aria-label="Preset quick cities">
                {PRESET_CITIES.map((c) => (
                  <button
                    key={c.name}
                    onClick={() => {
                    ...
                    aria-label={`Jump to preset city ${c.name}`}
                    className="px-2.5 py-1.5 rounded-lg bg-zinc-900/80 hover:bg-zinc-800 text-xs text-zinc-300 hover:text-white border border-zinc-800 transition"
                  >
                    {c.name.split(',')[0]}
                  </button>
                ))}
        ...
        {/* =========================================================================
           ...
           ========================================================================= */}
        <section aria-label="Navigation and Interactive Map Workspace" className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <form 
              onSubmit={(e) => { e.preventDefault(); executeRouting(); }} 
              role="search" 
              aria-label="Route Origin and Destination Search" 
              className="bg-[#101014] border border-zinc-800/80 rounded-2xl p-4 sm:p-5 space-y-3.5 shadow-xl"
            >
              <div className="text-xs font-bold uppercase tracking-wider text-emerald-400 flex items-center justify-between">
                ...
                <span className="font-mono text-xs text-zinc-400">OSRM Walk Graph</span>
              </div>

              {/* Origin Search */}
              <div className="relative">
                <div className="flex items-center justify-between mb-1.5">
                  <label htmlFor="origin-search-input" className="text-xs font-semibold text-emerald-400 flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5" aria-hidden="true" /> Start Point (Origin)
                  </label>
                  <button
                    type="button"
                    onClick={() => setClickMode('origin')}
                    aria-label="Set origin by clicking on map"
                    className={`text-xs px-2.5 py-1 rounded transition ${clickMode === 'origin' ? 'bg-emerald-500 text-white font-bold' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'}`}
                  >
                ...
                <div className="relative">
                  <input
                    ...
                    aria-label="Start point origin address or landmark"
                    value={originQuery}
                    onChange={(e) => handleOriginSearch(e.target.value)}
                    placeholder="Search origin landmark or address..."
                    className="w-full bg-zinc-900/90 border border-zinc-700/80 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 pr-9"
                  />
                  <Search className="w-4 h-4 text-zinc-400 absolute right-3 top-3 pointer-events-none" aria-hidden="true" />
                </div>
                  ...
                  <div className="absolute left-0 right-0 top-full mt-1 bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl z-50 max-h-48 overflow-y-auto divide-y divide-zinc-800">
                    {originSuggestions.map((p) => (
                      ...
                      >
                        ...
                        <div className="text-xs text-zinc-400 truncate">{p.displayName}</div>
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div className="relative">
                <div className="flex items-center justify-between mb-1.5">
                  <label htmlFor="dest-search-input" className="text-xs font-semibold text-rose-400 flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5" aria-hidden="true" /> Destination Point (End)
                  </label>
                  <button
                    type="button"
                    onClick={() => setClickMode('dest')}
                    aria-label="Set destination by clicking on map"
                    className={`text-xs px-2.5 py-1 rounded transition ${clickMode === 'dest' ? 'bg-rose-500 text-white font-bold' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'}`}
                  >
                ...
                <div className="relative">
                  <input
                    ...
                    type="text"
                    autoComplete="street-address"
                    aria-label="Destination point address or landmark"
                    value={destQuery}
                    onChange={(e) => handleDestSearch(e.target.value)}
                    placeholder="Search destination address..."
                    className="w-full bg-zinc-900/90 border border-zinc-700/80 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 pr-9"
                  />
                  <Search className="w-4 h-4 text-zinc-400 absolute right-3 top-3 pointer-events-none" aria-hidden="true" />
                </div>
              ...
              <div className="pt-3 border-t border-zinc-800/80 space-y-2">
                <div className="flex items-center justify-between text-xs text-zinc-300">
                  <label htmlFor="sun-time-slider" className="flex items-center gap-1.5 text-amber-400 font-semibold cursor-pointer">
                ...
                <input
                  ...
                  aria-label="Simulated Sun Time of Day"
                  aria-valuemin={6}
                  aria-valuemax={20}
                  aria-valuenow={timeSliderHours}
                  aria-valuetext={formattedSunTime}
                ...
                <div className="flex justify-between text-xs text-zinc-400 font-mono">
                  ...
                  <span>13:00 (Peak: {solar.elevationDeg}°)</span>
            ...
            {routingResult && (
              <div className="bg-[#101014] border border-zinc-800/80 rounded-2xl p-4 sm:p-5 space-y-3 shadow-xl">
                <div className="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center justify-between">
                  ...
                  {isLoadingRoute && <span className="text-emerald-400 animate-pulse text-xs">Computing...</span>}
                </div>
                ...
                <button
                  ...
                  aria-pressed={activeRouteType === 'shaded'}
                  aria-label={`Select Cool Walk Shaded Route, ${routingResult.coolRoute.averageShadePercent} percent shade, estimated time ${Math.round(routingResult.coolRoute.durationSeconds / 60)} minutes`}
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-bold text-white text-sm flex items-center gap-1.5">
                          ...
                          <span className="text-xs px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-300 font-mono font-bold">
                            RECOMMENDED
                          </span>
                        </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-emerald-900/40 text-xs font-mono">
                    <div className="flex items-center gap-1 text-emerald-300">
                      <Thermometer className="w-3.5 h-3.5 text-emerald-400" aria-hidden="true" />
                    ...
                    <div className="flex items-center gap-1 text-amber-300">
                      ...
                      <span><b>-{routingResult.coolRoute.solarRadiationReductionPercent}%</b> UV radiation</span>
                    </div>
                  </div>
                </button>
                ...
                <button
                  ...
                  aria-pressed={activeRouteType === 'direct'}
                  aria-label={`Select Direct Walk Route, ${routingResult.directRoute.averageShadePercent} percent shade, estimated time ${Math.round(routingResult.directRoute.durationSeconds / 60)} minutes`}
            ...
            {activeRouteData && (
              <div className="bg-[#101014] border border-zinc-800/80 rounded-2xl p-4 sm:p-5 space-y-2.5 max-h-72 overflow-y-auto shadow-xl">
                <div className="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-zinc-400 font-mono">Click step to zoom map</span>
                </div>
                {activeRouteData.steps.map((step, idx) => (
                  <button
                    ...
                    aria-label={`Step ${idx + 1}: ${step.instruction}, ${step.shadePercent}% shade`}
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <span className="w-5 h-5 rounded-full bg-zinc-800 flex items-center justify-center font-mono text-xs text-zinc-300 font-bold shrink-0 mt-0.5">
                        {idx + 1}
                      </span>
                      <div className="flex-1">
                        <div className="font-medium text-zinc-100">{step.instruction}</div>
                        <div className="flex items-center gap-2.5 mt-1 text-xs text-zinc-400 font-mono">
          ...
          <div className="lg:col-span-7 h-[540px] sm:h-[620px] lg:h-[720px] rounded-3xl overflow-hidden border border-zinc-800 relative shadow-2xl bg-zinc-950">
            ...
            <div ref={mapContainerRef} className="w-full h-full z-0" aria-label="Interactive Leaflet street and shade route map" />
            ...
            <div className="absolute top-4 right-4 z-20 bg-[#101014]/95 border border-zinc-800 rounded-2xl p-3.5 shadow-2xl backdrop-blur-md text-xs max-w-[240px] pointer-events-auto">
              <div className="flex items-center justify-between gap-2 pb-2 border-b border-zinc-800">
                ...
                <span className="font-mono text-emerald-400 font-bold bg-emerald-500/10 px-1.5 py-0.5 rounded text-xs">
                  LIVE
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 font-mono text-xs mt-2">
                <div>
                  <span className="text-zinc-400 block text-xs">Elevation:</span>
                  <span className="text-white font-bold">{solar.elevationDeg}°</span>
                </div>
                <div>
                  <span className="text-zinc-400 block text-xs">Azimuth:</span>
                  <span className="text-white font-bold">{solar.azimuthDeg}°</span>
                </div>
              </div>

              <div className="mt-2 pt-2 border-t border-zinc-800 text-xs text-zinc-400 flex items-center justify-between">
            ...
            <div className="absolute bottom-5 left-5 z-20 bg-[#101014]/95 border border-zinc-800 rounded-xl p-3 shadow-xl backdrop-blur-md text-xs space-y-1 pointer-events-auto">
              ...
              <div className="flex items-center gap-2 text-zinc-300 text-xs">
                <span className="w-3.5 h-1.5 bg-emerald-500 rounded-full" aria-hidden="true" />
                <span>Cool Shaded Route (Canopy + Shadows)</span>
              </div>
              <div className="flex items-center gap-2 text-zinc-300 text-xs">
                <span className="w-3.5 h-1.5 bg-amber-500 rounded-full" aria-hidden="true" />
        ...
        {/* =========================================================================
           ...
           ========================================================================= */}
        <section aria-labelledby="microclimate-heading" className="bg-[#101014] border border-zinc-800/80 rounded-3xl p-5 sm:p-7 space-y-6 shadow-2xl">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-zinc-800/80 pb-4">
            <div>
              <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-emerald-400">
                <Activity className="w-4 h-4" aria-hidden="true" /> Live Observational Atmospheric Telemetry
              </div>
              <h2 id="microclimate-heading" className="text-xl sm:text-2xl font-bold text-white mt-0.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-zinc-900/70 border border-zinc-800 p-5 rounded-2xl space-y-2">
                <div className="flex items-center justify-between text-zinc-400 text-xs">
                  <h3 className="flex items-center gap-1.5 text-zinc-200 font-medium text-xs">
                    <Thermometer className="w-4 h-4 text-rose-400" aria-hidden="true" />
                    Ambient Temperature
                  </h3>
                  <span className="font-mono text-emerald-400 font-bold text-xs">LIVE</span>
                </div>
              ...
              <div className="bg-zinc-900/70 border border-zinc-800 p-5 rounded-2xl space-y-2">
                <div className="flex items-center justify-between text-zinc-400 text-xs">
                  ...
                  <span className="font-mono text-blue-400 text-xs">Astronomical</span>
                </div>
              ...
              <div className="bg-zinc-900/70 border border-zinc-800 p-5 rounded-2xl space-y-2">
                <div className="flex items-center justify-between text-zinc-400 text-xs">
                  ...
                  <span className="font-mono text-cyan-400 text-xs">{weather.weatherDescription}</span>
                </div>
          ...
          {weather && weather.hourly.time.length > 0 && (
            <div className="bg-zinc-900/80 border border-zinc-800 p-5 rounded-2xl space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-zinc-400 font-mono">Hourly Progression</span>
              </div>
                ...
                {weather.hourly.time.map((t, idx) => {
                  ...
                  return (
                    <div key={idx} className="bg-zinc-800/70 border border-zinc-700/60 p-2.5 rounded-xl text-center space-y-1 min-w-[85px]">
                      <div className="text-xs font-mono text-zinc-300 font-bold">{hourLabel}</div>
                      <div className="text-base font-bold text-white font-mono">{temp}°C</div>
                      <div className={`text-xs font-mono px-1.5 py-0.5 rounded font-bold ${
        ...
        {/* =========================================================================
           ...
           ========================================================================= */}
        <section aria-labelledby="science-heading" className="bg-[#101014] border border-zinc-800/80 rounded-3xl p-5 sm:p-7 space-y-5 shadow-2xl">
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-rose-400">
            <Activity className="w-4 h-4" aria-hidden="true" />
            <h2 id="science-heading" className="font-bold text-rose-400 text-xs uppercase tracking-wider">
              The Science of Urban Heat & Mean Radiant Temperature (T_mrt)
            </h2>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="p-5 rounded-2xl bg-zinc-900/60 border border-zinc-800 space-y-2">
              <div className="w-8 h-8 rounded-lg bg-blue-500/20 text-blue-400 flex items-center justify-center font-bold text-xs" aria-hidden="true">
                1
              </div>
              <h3 className="text-sm font-bold text-white">Mean Radiant Temperature (MRT)</h3>
            ...
            <div className="p-5 rounded-2xl bg-zinc-900/60 border border-zinc-800 space-y-2">
              <div className="w-8 h-8 rounded-lg bg-amber-500/20 text-amber-400 flex items-center justify-center font-bold text-xs" aria-hidden="true">
                2
              </div>
              <h3 className="text-sm font-bold text-white">Building Canyon Geometry</h3>
              <p className="text-xs text-zinc-300 leading-relaxed">
                We calculate street shadow length via <span className="font-mono text-amber-300">L = H / tan(α)</span> and street-sun angle divergence θ, determining exactly which sidewalk side remains sheltered.
              </p>
            </div>

            <div className="p-5 rounded-2xl bg-zinc-900/60 border border-zinc-800 space-y-2">
              <div className="w-8 h-8 rounded-lg bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold text-xs" aria-hidden="true">
                3
              </div>
              <h3 className="text-sm font-bold text-white">Open Data Pipeline</h3>
              <p className="text-xs text-zinc-300 leading-relaxed">
